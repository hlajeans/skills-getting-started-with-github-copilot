  ...
  async function fetchActivities() {
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
              ...
              <ul class="participants-list">
                ${details.participants.map(p => `
                  <li style="list-style-type:none; display:flex; align-items:center; gap:6px;">
                    <span>${p}</span>
                    <button class="delete-participant-btn" title="Remove participant" data-activity="${name}" data-email="${p}" style="background:none;border:none;color:#c62828;cursor:pointer;font-size:16px;padding:2px 6px;">
        ...
        // Agregar listeners a los botones de eliminar después de renderizar
        setTimeout(() => {
